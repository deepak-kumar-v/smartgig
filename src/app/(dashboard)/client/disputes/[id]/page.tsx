...
export default function ClientDisputeDetailPage() {
    ...
    const { dispute, contract, milestone, lockAmount, currentUserId, isAdmin, walletBalance } = data;
                            ...
                            <>
                                ...
                                <div className="flex gap-2">
                                    ...
                                    <button
                                        ...
                                        className="px-4 py-2 bg-indigo-500/20 text-indigo-400 rounded-lg hover:bg-indigo-500/30 disabled:opacity-40 transition-colors"
                                    >
                                ...
                                {evidence.map((ev) => (
                                    <div key={ev.id} className="flex items-center justify-between p-3 bg-zinc-800/50 rounded-lg border border-zinc-700/50">
                                        ...
                                        <span className="text-xs text-zinc-600">
                                            {ev.uploadedById === currentUserId ? 'You' : 'Other party'} • {new Date(ev.createdAt).toLocaleDateString()}
                                        </span>
                                    </div>
                                ))}
                            </div>
                    ...
                    <GlassCard className="p-5">
                        ...
                        <div className="space-y-3">
                            <div className="flex justify-between text-sm">
                                ...
                                <span className="text-white">{reasonLabels[dispute.reason] || dispute.reason}</span>
                            </div>
                            <div className="flex justify-between text-sm">
                            ...
                            {dispute.resolvedAt && (
                                <div className="flex justify-between text-sm">
                                    ...
                                    <span className="text-emerald-400">{new Date(dispute.resolvedAt).toLocaleDateString()}</span>
                                </div>
                            )}
                            {dispute.outcome && dispute.freelancerPercent !== null && (
                                <div className="mt-3 pt-3 border-t border-zinc-800">
                                    <p className="text-emerald-400 text-sm font-medium">
                                        Resolution: {dispute.freelancerPercent}% to freelancer
                                    </p>
                                    {dispute.resolutionNote && (
                                        <p className="text-zinc-500 text-xs mt-1">{dispute.resolutionNote}</p>
                                    )}
                                </div>
                            )}
                        </div>
                                    ...
                                    <>
                                        {!showProposal ? (
                                            <button
                                                ...
                                                className="w-full px-4 py-2 bg-violet-500/20 text-violet-400 rounded-lg hover:bg-violet-500/30 transition-colors text-sm"
                                            >
                                                Submit Proposal
                                            </button>
                                        ) : (
                                            <div className="space-y-3 p-3 bg-zinc-800/50 rounded-lg border border-zinc-700/50">
                                                ...
                                                <div>
                                                    ...
                                                    <input
                                                        ...
                                                        onChange={e => setProposalPercent(Number(e.target.value))}
                                                ...
                                                <textarea
                                                    ...
                                                    rows={2}
                                                    className="w-full bg-zinc-900 border border-zinc-700 rounded px-2 py-1 text-sm text-white resize-none focus:outline-none focus:border-violet-500"
                                                />
                                                <div className="flex gap-2">
                                                    <button
                                                        onClick={() => setShowProposal(false)}
                                                        className="flex-1 px-3 py-1.5 border border-zinc-700 text-zinc-400 rounded text-sm hover:text-white transition-colors"
                                                    >
                                                        Cancel
                                                    </button>
                                                    <button
                                                        ...
                                                        disabled={isPending}
                                                        className="flex-1 px-3 py-1.5 bg-violet-500/20 text-violet-400 rounded text-sm hover:bg-violet-500/30 disabled:opacity-40 transition-colors"
                                                    >
