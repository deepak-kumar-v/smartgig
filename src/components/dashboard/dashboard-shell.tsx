...
import { LayoutDashboard, Briefcase, FileText, MessageSquare, Settings, LogOut, Bell, Search, Menu, User, Image, Shield, Users, AlertTriangle } from 'lucide-react';
...
const navItems = {
    freelancer: [
        ...
        { name: 'Find Work', href: '/explore', icon: Search },
...
export function DashboardShell({ children, role = 'freelancer' }: DashboardShellProps) {
    ...
    return (
        <div className="min-h-screen bg-background flex">
            {/* Sidebar Desktop */}
            <aside className="hidden lg:flex flex-col w-64 border-r border-white/5 bg-white/5 backdrop-blur-xl h-screen sticky top-0">
                <div className="p-6">
                    <Logo />
                </div>
                ...
                {sidebarOpen && (
                    ...
                    >
                        <div className="flex justify-between items-center mb-8">
                            <Logo />
                            <button onClick={() => setSidebarOpen(false)}><LogOut className="w-5 h-5 text-white/50" /></button>
                        </div>
                ...
                <header className="h-16 border-b border-white/5 bg-background/50 backdrop-blur-md sticky top-0 z-40 px-6 flex items-center justify-between">
                    ...
                    <div className="flex-1 max-w-xl mx-4 hidden md:block">
                        <div className="relative">
                            ...
                            <input type="text" placeholder="Search jobs, clients, contracts..." className="w-full bg-white/5 border border-white/10 rounded-full pl-10 pr-4 py-1.5 text-sm text-white focus:outline-none focus:border-white/20 transition-all" />
                        </div>
                    </div>
